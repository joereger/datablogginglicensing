...
import reger.core.agotext.OldAgoText;
import reger.core.agotext.AgoText;
...
//---------------------------------------------------------------
	...
	*/
	public static Calendar formtocalendar(int yyyy, int mm, int dd, int h, int m, int s, String ampm) {
		...
		Calendar date = TimeUtils.dbstringtocalendar(yyyy + "- " + mmStr + "-" + ddStr + " " + hStr + ":" + mStr + ":" + sStr);
	...
	*/
	public static String agoText(Calendar indate){
	    AgoText at = new AgoText();
        return at.formatPrecise(indate.getTime());
	    //return OldAgoText.agoText(indate);
	}
...
//		return OldAgoText.agoTextEnd(diff, unit);
//	}
...
//		return OldAgoText.agoTextSuffix(diff);
//	}
//
//	/**
...
//		return OldAgoText.agoTextEss(diff);
//	}
	...
	*/
	public static String agoGraphicText(Calendar indate){
        ...
        if (Util.qAbs(daysago) < 31) {
            //Days
            result = Util.qAbs(daysago) + "daysago.gif";
        } else {
            if (Util.qAbs(weeksago) < 4) {
                //Weeks
                result = Util.qAbs(weeksago) + "weeksago.gif";
            } else {
                if (Util.qAbs(monthsago) < 12) {
                    //Months
                    result = Util.qAbs(monthsago) + "monthsago.gif";
                } else {
                    //Years
                    result = Util.qAbs(yearsago) + "yearsago.gif";
                }
            }
        }
...
//     }
    ...
    public static Calendar getRandomDateInPast(int maxAgeInDays){
        ...
        Calendar tmpDate = TimeUtils.nowInGmtCalendar();
        return TimeUtils.xMinutesAgoStart(tmpDate, Util.randomInt(maxAgeInMinutes));
    }
